...
public void deshacerUltimoCambio(int nroTramite) throws CambioEstadoException {
    ...
    try {
        ...
        // Ordenar el historial de estados por fecha 'fechaDesdeTET'
        ...
        // Obtener la fecha actual y añadir un margen de 1 minuto
        Timestamp fechaActual = fechaHoraActual.obtenerFechaHoraActual();
        Timestamp nuevaFechaHasta = new Timestamp(fechaActual.getTime() + 1 * 60 * 1000);

        // Actualizar fecha "hasta" del último estado y ajustar la fecha "desde" si es necesario
        ultimoCambio.setFechaHastaTET(nuevaFechaHasta);

        // Evitar que el estado anterior y el nuevo tengan fechas coincidentes
        if (estadoAnterior.getFechaHastaTET() != null && 
            estadoAnterior.getFechaHastaTET().equals(ultimoCambio.getFechaDesdeTET())) {
            estadoAnterior.setFechaDesdeTET(new Timestamp(fechaActual.getTime() - 1 * 60 * 1000));
        }

        // Guardar cambios
        FachadaPersistencia.getInstance().guardar(ultimoCambio);
        FachadaPersistencia.getInstance().guardar(tramite);
